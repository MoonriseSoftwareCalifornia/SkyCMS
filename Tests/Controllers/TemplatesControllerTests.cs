...
namespace Sky.Tests.Controllers
{
    ...
    public class TemplatesControllerTests : SkyCmsTestBase
    {
        ...
        #region Phase 4b: Designer and Preview Endpoint Tests

        /// <summary>
        /// Tests that Designer returns NotFound for a missing template.
        /// </summary>
        [TestMethod]
        ...
        /// <summary>
        /// Tests that GetDesignerData returns NotFound for a missing template.
        /// </summary>
        [TestMethod]
        public async Task GetDesignerData_ReturnsNotFound_WhenTemplateMissing()
        {
            // Act
            var result = await _controller.GetDesignerData(Guid.NewGuid());
        ...
        /// <summary>
        /// Tests that DesignerData returns BadRequest with an empty template id.
        /// </summary>
        [TestMethod]
        public async Task DesignerData_ReturnsBadRequest_WhenTemplateIdIsEmpty()
        {
            // Act
            var result = await _controller.DesignerData(Guid.Empty, "title", string.Empty, string.Empty);
        ...
        /// <summary>
        /// Tests that PreviewImpact redirects to index when template is missing.
        /// </summary>
        [TestMethod]
        public async Task PreviewImpact_RedirectsToIndex_WhenTemplateMissing()
        {
            // Act
            var result = await _controller.PreviewImpact(Guid.NewGuid());
        ...
        /// <summary>
        /// Tests that PreviewImpactJson returns an error payload when template is missing.
        /// </summary>
        [TestMethod]
        public async Task PreviewImpactJson_ReturnsErrorPayload_WhenTemplateMissing()
        {
            // Act
            var result = await _controller.PreviewImpactJson(Guid.NewGuid());
        ...
        /// <summary>
        /// Tests that PublishDrafts redirects back to Pages for missing template.
        /// </summary>
        [TestMethod]
        public async Task PublishDrafts_RedirectsToPages_WhenTemplateMissing()
        {
            // Act
            var result = await _controller.PublishDrafts(Guid.NewGuid(), new System.Collections.Generic.List<int> { 1, 2 });

            // Assert
            Assert.IsInstanceOfType(result, typeof(RedirectToActionResult));
            var redirect = (RedirectToActionResult)result;
            Assert.AreEqual("Pages", redirect.ActionName);
        }
