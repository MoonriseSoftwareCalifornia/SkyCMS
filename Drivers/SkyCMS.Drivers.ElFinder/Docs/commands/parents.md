...
- **Ancestor chain**: The `tree[]` array must include every ancestor from root to target with siblings at each level — the same contract as `open`'s `files[]`. See [phash chain contract](../skycms-implementation-notes.md#phash-chain-contract--commands-that-must-return-the-full-ancestor-path) for the full rule and a reference diagram.
